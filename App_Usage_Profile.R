...
server <- function(input, output) {
  ...
  output$table1<-DT::renderDataTable(DT::datatable({fetchData %>% select(APPLICATION_NAME,ACTIVITY_NAME,SERVER_NAME,SERVER_HOSTNAME,SERVER_IP) %>% distinct(.keep_all = TRUE) %>% arrange(APPLICATION_NAME,SERVER_NAME)
  },options = list(searching = TRUE, paging= TRUE,scrollX = TRUE)))
